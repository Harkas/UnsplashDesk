/*
* @Author: Andrey
* @Date:   2017-05-05 08:48:04
* @Last Modified by:   Andrey
* @Last Modified time: 2017-05-06 11:31:19
*/
...
function getCurDate()
{
 ...
 var ntime = hours+":"+minutes+":"+seconds;
 var ndayweek = years+"年"+month+"月"+days+"日 "+ " "+week;
 divTime.innerHTML= ntime;
 divDayWeek.innerHTML= ndayweek;
}
...
function RandomQuote()
    {
        var RandomQuoteNumber = Math.floor(Math.random() * tips.length);
        document.getElementById('quotedisplay').innerHTML= tips[RandomQuoteNumber];
     }
// 设定条目随机切换时间，单位为毫秒，此函数功能只在页面打开时执行一次。
setTimeout("RandomQuote()",0)
 // 设定条目切换时间，单位为毫秒。 1秒=1000毫秒。此函数根据设定的间隔时间持续执行
 setInterval("RandomQuote()",30000);
//指定显示条目为下面条目的总数量
 tips = new Array(length);
//条目内容
...
tips[100] = '学会偷懒，并懒出境界是提高工作效率最有效的方法！';
...
tips[107] = '孤独的时刻，和自己一起成长 ';
tips[108] = '知耻近乎勇。 —— 孔丘';
tips[109] = '辱，莫大于不知耻。 —— 王通';
tips[110] = '君子忧道不忧贫。 —— 孔丘';
tips[111] = '万物皆有裂痕，那是光进来的地方。 —— 莱昂纳德·科恩';
tips[112] ='There is a Crack in Everything, That‘s How the Light Gets in。—— Leonard Cohen ';
